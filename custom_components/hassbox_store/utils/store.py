...
from ..const import VERSION_STORAGE, DOMAIN
...
def _get_store_for_key(hass, key, encoder):
    ...
    return CustomStore(hass, VERSION_STORAGE, key, encoder=encoder, atomic_writes=True)
...
async def async_save_to_store(hass, key, data):
    ...
    if current is None or current != data:
        ...
        return
    _LOGGER.debug(
        "<HassBox Store async_save_to_store> Did not store data for '%s'. Content did not change",
        key,
    )
